...
export default function App() {
  ...
  // ---------------- Health Records ----------------
  const healthRecords = [
    { date: "10 Aug", desc: "Paracetamol 500mg", type: "Prescription" },
    { date: "02 Aug", desc: "Consultation: Fever, advised rest", type: "Consultation" },
    { date: "28 Jul", desc: "Blood Test: Normal", type: "Test" },
    { date: "15 Jul", desc: "Ibuprofen 200mg", type: "Prescription" },
    { date: "01 Jul", desc: "Stomach ache dietary advice", type: "Consultation" },
    { date: "20 Jun", desc: "X-Ray: No issues", type: "Test" },
    { date: "05 Jun", desc: "Vitamin D Supplements", type: "Prescription" },
  ];
  ...
  const symptomResponses = {
    en: {
      urgent: "⚠ Urgent: Go to hospital",
      seeDoctor: "🤒 See a doctor soon",
      mild: "🙂 Mild, manage at home",
      unknown: "🙂 Symptoms not recognized, monitor & consult if needed",
      empty: "Please type something first...",
    },
    hi: {
      urgent: "⚠ तुरंत अस्पताल जाएँ",
      seeDoctor: "🤒 डॉक्टर से जल्द मिलें",
      mild: "🙂 हल्का, घर पर संभालें",
      unknown: "🙂 लक्षण पहचाने नहीं गए, जरूरत होने पर डॉक्टर से मिलें",
      empty: "कृपया कुछ टाइप करें...",
    },
    ta: {
      urgent: "⚠ அவசரமாக: மருத்துவமனைக்கு செல்லவும்",
      seeDoctor: "🤒 டாக்டரை விரைவில் பார்க்கவும்",
      mild: "🙂 லேசானது, வீட்டில் பராமரிக்கவும்",
      unknown: "🙂 அறிகுறிகள் தெரியவில்லை, தேவையெனில் மருத்துவரை பார்க்கவும்",
      empty: "தயவுசெய்து ஏதாவது টাইப் செய்யவும்",
    },
    ml: {
      urgent: "⚠ അടിയന്തരമായി: ആശുപത്രിയിലേക്ക് പോകുക",
      seeDoctor: "🤒 ഡോക്ടറെ ഉടനെ കാണുക",
      mild: "🙂 ലളിതം, വീട്ടിൽ നിയന്ത്രിക്കുക",
      unknown: "🙂 ലക്ഷണങ്ങൾ തിരിച്ചറിയാനായില്ല, ആവശ്യമെങ്കിൽ ഡോക്ടറെ കാണുക",
      empty: "ദയവായി ഒന്നുകൂടി ടൈപ്പ് ചെയ്യുക",
    },
    te: {
      urgent: "⚠ అత్యవసరం: ఆసుపత్రికి వెళ్లండి",
      seeDoctor: "🤒 డాక్టర్‌ను త్వరగా చూడండి",
      mild: "🙂 లైట్, ఇంట్లో నిర్వహించండి",
      unknown: "🙂 లక్షణాలు గుర్తించబడలేదు, అవసరమైతే డాక్టర్‌ను చూడండి",
      empty: "దయచేసి ఏదైనా టైప్ చేయండి...",
    },
  };
  ...
  const handleLogin = () => {
    if (!name || !password || !otpInput) {
      alert("Please fill all fields");
      return;
    }
    if (otpInput.trim() === "1234") switchScreen("home");
    else alert("Wrong OTP! (hint: 1234 😅)");
  };

  const checkSymptoms = () => {
    let text = (symptomNote || "").trim().toLowerCase();
    if (!text || text.length < 2) {
      setSymptomOutput(symptomResponses[language].empty);
      return;
    }
    if (text.includes("pain") || text.includes("shortness of breath"))
      setSymptomOutput(symptomResponses[language].urgent);
    else if (text.includes("fever") || text.includes("rash"))
      setSymptomOutput(symptomResponses[language].seeDoctor);
    else if (text.includes("cough") || text.includes("headache") || text.includes("stomach"))
      setSymptomOutput(symptomResponses[language].mild);
  ...
  const containerStyle = {
    minHeight: "100vh",
    background: "linear-gradient(to bottom, #e6f4ea, #e6f0fa)",
  ...
  const statusTag = (status) => {
    const color = status === "available" ? "#10b981" : status === "low" ? "#f59e0b" : "#ef4444";
    return { color: "#fff", background: color, padding: "2px 6px", borderRadius: 6, fontSize: 12, marginLeft: 8 };
  };

  const recordTag = (type) => {
    const color = type === "Prescription" ? "#3b82f6" : type === "Consultation" ? "#f97316" : "#10b981";
    return { color: "#fff", background: color, padding: "2px 6px", borderRadius: 6, fontSize: 12, marginLeft: 8 };
  };
      ...
      <div style={contentStyle}>
        ...
        {activeScreen === "login" && (
          <div style={card}>
            ...
            <input placeholder={texts[language].name} value={name} onChange={e => setName(e.target.value)} style={inputStyle} />
            ...
            <button 
              ...
              style={{
                ...
                border: "none", background: "linear-gradient(to right,#4f46e5,#6366f1)",
                color: "#fff", fontWeight: 600, cursor: "pointer", marginTop: 10
              }}
            >
            ...
            <div style={{ marginTop: 20, textAlign:"center" }}>
              {["en","hi","ta","ml","te"].map(lang=>(
                <button key={lang} onClick={()=>setLanguage(lang)} style={{margin:4,padding:"4px 8px",borderRadius:6,border:"1px solid #ccc",background: language===lang?"#6366f1":"#fff", color: language===lang?"#fff":"#000"}}>
                  {lang.toUpperCase()}
                </button>
              ))}
            </div>
        ...
        {activeScreen === "home" && (
          <div style={{ ...card, textAlign: "center", position: "relative", overflow: "hidden" }}>
            ...
            <h2 style={{
              color: "#166534", fontSize: 28, marginBottom: 24,
              opacity: animate ? 1 : 0, transform: animate ? "translateY(0)" : "translateY(-20px)",
              transition: "all 0.6s ease"
            }}>
              {texts[language].loginTitle}, {name}
            </h2>
            ...
            <div style={{ display: "grid", gap: 20 }}>
              {[ 
                ...
                <div key={i} onClick={()=>switchScreen(btn.screen)} style={{
                  background: btn.bg,
                  padding:30,
                  borderRadius:20,
                  fontSize:20,
          ...
          <div style={card}>
            ...
            {doctors.map((doc,i)=>(
              <div key={i} style={listCardStyle}>
                ...
                <div>{doc.spec}</div>
                <button style={{padding:"4px 8px", borderRadius:6, border:"none", background:"#6366f1", color:"#fff"}}>{texts[language].book}</button>
              </div>
            ))}
